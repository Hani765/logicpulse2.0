...
const CreateForm: React.FC<TagInputProps> = ({ domains, token, users, networks, countries, response }) => {
    ...
    const [offerState, setOfferState] = useState<OfferState>({
        offer_name: response.offer.offer_name,
        platform_name: response.offer.platform_name,
        age: response.offer.age,
        rate: response.offer.rate,
        encryption: response.offer.encryption,
        network_id: response.offer.network_id,
        domain_id: response.offer.domain_id,
        proxy: response.offer.proxy,
        details: response.offer.details,
        status: response.offer.status,
        users_ids: response.users,
        countries: response.offer.countries,
        urls: response.offer.urls,
    });
    ...
    const submitOffer = async (event: React.FormEvent) => {
        ...
        setLoading(true);
        const endPoint = `${API_URL + OFFERS}/${response.offer.unique_id}`;
        myAxios.put(endPoint, offerState, {
            headers: {
